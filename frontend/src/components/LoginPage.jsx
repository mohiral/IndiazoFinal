...
import { FaUserCircle, FaLock, FaMobile, FaSpinner } from "react-icons/fa"
...
const LoginPage = () => {
  ...
  const fetchContactInfo = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5001/api/contacts/active")
      const data = await response.json()

      if (data.success && data.contacts) {
        // Extract phone and whatsapp from contacts
        let phone = ""
        let whatsapp = ""
        ...
        console.log("Fetched contact info:", { phone, whatsapp })
      }
    } catch (error) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/users?mobile=${formData.mobile}`)
      ...
      if (existingUser) {
        ...
        setTimeout(() => {
          window.location.href = "/" // Use direct URL navigation instead of React Router
        }, 300)
      } else {
  ...
  const handleWhatsApp = () => {
    if (contactInfo.whatsapp) {
      ...
      // Open in a new tab to avoid any navigation issues
