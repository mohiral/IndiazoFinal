...
export default function WalletTransactions() {
  ...
  // Update the transaction mapping in useEffect to force pending status
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        console.log("Original transaction data:", data)

        // Process the data to handle withdrawal requests correctly
        const processedData = data.map((transaction) => {
          // If it's a withdrawal request that has been deducted
          if (
            transaction.transactionType === "withdrawal" &&
            transaction.amount < 0 &&
            transaction.deductedFromBalance
          ) {
            // ALWAYS force status to pending unless explicitly confirmed or rejected
            if (transaction.status !== "confirmed" && transaction.status !== "rejected") {
              return {
                ...transaction,
                status: "pending", // Force status to be pending
                displayStatus: "pending", // Show as pending in user's transaction history
              }
            }
          }
          return transaction
  ...
  const filteredTransactions = transactions
    ...
    .filter((transaction) => {
      ...
      if (filterType === "pending") return transaction.status === "pending" || transaction.adminReview
  ...
  const getTransactionStats = () => {
    ...
    const pendingWithdrawals = withdrawals.filter((t) => t.status === "pending" || t.adminReview)
  ...
  const mockStats = {
    ...
    pendingWithdrawalAmount: 200,
  }

  const displayStats = useMockData ? mockStats : stats

  // Update the getStatusLabel function to ALWAYS show withdrawals with deductedFromBalance=true as pending
  // unless they are explicitly confirmed or rejected

  const getStatusLabel = (status, adminReview, transaction) => {
    // Check adminApproval field first for withdrawals
    if (
      transaction &&
      transaction.transactionType === "withdrawal" &&
      transaction.amount < 0 &&
      ...
      // Use adminApproval status if available
      if (transaction.adminApproval === "pending") {
        return "Pending"
      } else if (transaction.adminApproval === "confirmed" || transaction.adminApproval === "approved") {
        return "Confirmed"
      } else if (transaction.adminApproval === "rejected") {
        return "Rejected"
      }
    }

    // Fallback to regular status if adminApproval is not available
    if (status === "pending") {
      return "Pending"
    }
  ...
  // Update the getStatusIcon function to handle deductedFromBalance
  const getStatusIcon = (status, adminReview, transaction) => {
    // Check adminApproval field first for withdrawals
    if (
      transaction &&
      transaction.transactionType === "withdrawal" &&
    ...
    // Fallback to regular status checks
    if (adminReview) {
  ...
  // Update the getStatusClass function to handle deductedFromBalance
  const getStatusClass = (status, adminReview, transaction) => {
    // Check adminApproval field first for withdrawals
    if (
      transaction &&
      transaction.transactionType === "withdrawal" &&
  ...
  // Update the transaction card rendering to pass the transaction to status functions
  const displayTransactionsMapped = displayTransactions.map((transaction, index) => {
    ...
    const adminReview = transaction.adminReview || false

    // Determine display status based on adminApproval for withdrawals
    let displayStatus = transaction.status
    if (transaction.transactionType === "withdrawal" && transaction.amount < 0 && transaction.adminApproval) {
      // Use adminApproval status if available
      displayStatus = transaction.adminApproval
    }
    ...
    return (
      <motion.div
        ...
        className={`bg-gradient-to-r ${getStatusClass(displayStatus, adminReview, transaction)} rounded-lg p-4 shadow-lg`}
      >
        <div className="flex flex-col md:flex-row justify-between">
          <div>
            <div className="flex items-center">
              ...
              >
                {getStatusIcon(displayStatus, adminReview, transaction)}
                <span className="capitalize">{getStatusLabel(displayStatus, adminReview, transaction)}</span>
              </span>
              <span className="ml-2 text-gray-300 text-sm">{new Date(transaction.createdAt).toLocaleString()}</span>
            </div>
          ...
          <div className="mt-3 md:mt-0 text-right">
            ...
            <div className="text-xs text-gray-400 mt-1">
              {isWithdrawal && transaction.adminApproval === "pending"
                ? "Amount deducted - Awaiting admin approval"
                : displayStatus === "pending"
                  ? isWithdrawal
                    ? "Amount deducted - Awaiting admin approval"
                    : "Processing..."
  ...
  const activeStats = {
    label: activeTab === "deposits" ? "Deposit" : activeTab === "withdrawals" ? "Withdrawal" : "Transaction",
    total:
      activeTab === "deposits"
        ? displayStats.totalDeposits
        : activeTab === "withdrawals"
          ? displayStats.totalWithdrawals
          : displayStats.totalDeposits + displayStats.totalWithdrawals,
    confirmed:
      activeTab === "deposits"
        ? displayStats.confirmedDeposits
        : activeTab === "withdrawals"
          ? displayStats.confirmedWithdrawals
          : displayStats.confirmedDeposits + displayStats.confirmedWithdrawals,
    pending:
      activeTab === "deposits"
        ? displayStats.pendingDeposits
        : activeTab === "withdrawals"
          ? displayStats.pendingWithdrawals
          : displayStats.pendingDeposits + displayStats.pendingWithdrawals,
    totalAmount:
      activeTab === "deposits"
        ? displayStats.totalDepositAmount
        : activeTab === "withdrawals"
          ? displayStats.totalWithdrawalAmount
          : displayStats.totalDepositAmount + displayStats.totalWithdrawalAmount,
    pendingAmount:
      activeTab === "deposits"
        ? displayStats.pendingDepositAmount
        : activeTab === "withdrawals"
          ? displayStats.pendingWithdrawalAmount
          : displayStats.pendingDepositAmount + displayStats.pendingWithdrawalAmount,
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-black to-blue-900 text-white pt-16 pb-20">
      <div className="container mx-auto px-4 py-6">
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">{displayTransactionsMapped}</div>
        )}
