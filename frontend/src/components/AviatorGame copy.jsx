    ...
    >
      ...
      <div className="w-full bg-gray-900/70 mb-2 p-2 rounded flex flex-wrap justify-center gap-2">
        ...
        {lastCrashes.map((crash, index) => (
          <span
            key={index}
            className={`text-sm font-semibold ${crash <= 1.2 ? "text-red-500" : crash <= 2 ? "text-orange-400" : "text-green-400"
              }`}
          >
            ...
            >
              ...
              >
                {[...Array(36)].map((_, i) => (
                  <motion.div
                    key={i}
                    className={`absolute w-2 h-2 rounded-full ${i % 3 === 0 ? "bg-yellow-400" : i % 3 === 1 ? "bg-orange-500" : "bg-red-600"
                      }`}
                    initial={{ x: 0, y: 0 }}
                    animate={{
