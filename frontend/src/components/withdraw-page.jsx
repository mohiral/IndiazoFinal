...
const WithdrawPage = () => {
  ...
  const fetchWalletBalance = async (userId) => {
    try {
      ...
      // Validate initial amount after getting wallet balance
      validateAmount(amount, data.balance)
    } catch (error) {
  ...
  const handleAmountChange = (e) => {
    ...
    setAmount(value)
    validateAmount(value, walletBalance)
  }

  const validateAmount = (value, balance = walletBalance) => {
    if (value < 500) {
  ...
  // Update the handleSubmit function to reflect the immediate balance deduction
  const handleSubmit = async (e) => {
    ...
    const withdrawalData = {
      ...
      deductedFromBalance: true, // This flag indicates the amount is already deducted
    ...
    try {
      ...
      if (response.ok) {
        // Check if the API returned the correct status
        if (result.status && result.status !== "pending") {
          console.error("Warning: API returned non-pending status:", result.status)
        }

        // Update local wallet balance immediately to reflect the deduction
  ...
  // Calculate maximum amount user can withdraw
  const maxWithdrawalAmount = Math.floor(walletBalance)

  // Filter preset amounts to only show those the user can afford
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-4 gap-2 mt-3">
                  {affordablePresetAmounts.map((presetAmount) => (
                    <button
                      type="button"
                      key={presetAmount}
                ...
                <div className="mt-3 text-sm text-gray-600 flex items-center">
                  ...
                  Available for withdrawal: <span className="font-bold ml-1">₹{maxWithdrawalAmount}</span>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  required
                />
                <p className="text-xs text-gray-500 mt-1">Enter the UPI ID where you want to receive the money</p>
              </div>
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  required
                />
                <p className="text-xs text-gray-500 mt-1">Name should match with your bank account</p>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                ...
                <ul className="text-sm text-blue-700 space-y-1">
                  ...
                  <li>• Maximum withdrawal amount is your wallet balance</li>
                  <li>• Withdrawals are processed within 24-48 hours</li>
                  <li>• Amount will be deducted immediately from your wallet</li>
                  <li>• Amount will not be refunded if request is rejected</li>
              ...
              <button
                type="submit"
                disabled={isSubmitting || !isAmountValid || amount > walletBalance || amount < 500}
                className={`w-full py-3 px-4 rounded-lg font-medium flex items-center justify-center ${
                  isSubmitting || !isAmountValid || amount > walletBalance || amount < 500
