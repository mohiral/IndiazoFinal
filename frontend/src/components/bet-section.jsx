...
export default function BetSection({
  ...
  useEffect(() => {
    if (gameId) {
      console.log(`Section ${id} - Game ID updated:`, gameId)
  ...
  useEffect(() => {
    const fetchWalletBalances = async () => {
      ...
      try {
        ...
        console.error(`Section ${id} - Error fetching wallet balances:`, error)
  ...
  useEffect(() => {
    if (gameState === "crashed" && hasBet) {
      ...
      console.log(`Section ${id} - Game crashed with gameId:`, currentGameId.current)
  ...
  }, [gameState, hasBet, betAmount, confirmedBalance, autoBetAmount, id])
  ...
  const updateGameBalance = async (userId, newBalance, gameResult, betAmount, winAmount, gameId) => {
    try {
      ...
      const data = await response.json()
      console.log(`Section ${id} - Server response:`, data)
      ...
      console.error(`Section ${id} - Error updating game balance:`, error)
      showError("Failed to update balance")
      return { error: true, message: error.message || "Failed to update balance" }
    }
  }
  ...
  const handlePlaceBet = async () => {
    ...
    try {
      const userId = JSON.parse(user).userId
      const result = await onPlaceBet(betAmount, userId, id)
      ...
      console.error(`Section ${id} - Error placing bet:`, error)
  ...
  const handleDirectAutoBet = async () => {
    if (isPlacingBet || gameState === "active" || hasBet || gameState !== "waiting") return

    // Use the auto bet amount for direct auto betting
    setIsPlacingBet(true)
    clearMessage()
    ...
    try {
      const userId = JSON.parse(user).userId
      const result = await onPlaceBet(autoBetAmount, userId, id)
      ...
      console.error(`Section ${id} - Error placing auto bet:`, error)
  ...
  const handleCancelBet = async () => {
    ...
    try {
      ...
      console.error(`Section ${id} - Error cancelling bet:`, error)
  ...
  const handleCashOut = async () => {
    ...
    try {
      ...
      console.log(`Section ${id} - Cash out with gameId:`, currentGameId.current)

      // Add a check to ensure gameId is valid before proceeding
      if (!currentGameId.current) {
        showError("Invalid game ID. Please refresh the page.")
        return
      }

      const result = await onCashOut(userId, currentBetAmount, id)

      // Improved error handling with more detailed logging
      if (!result) {
        console.error(`Section ${id} - Cash out failed: No result returned`)
      ...
      const newBalance = confirmedBalance + totalWinAmount
      setConfirmedBalance(newBalance)
      setHasBet(false)
      ...
      console.error(`Section ${id} - Error cashing out:`, error)
  ...
  return (
    <div className="w-full max-w-3xl bg-[#1a1a1a] rounded-lg p-2 mb-4">
      {/* Section Title */}
      {/* <div className="text-white text-xs font-bold mb-1 border-b border-gray-800 pb-0.5">Section {id}</div> */}
      ...
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex">
          <button
            className={`px-2 py-0.5 text-xs rounded-l-lg transition-colors ${
          ...
          <button
            className={`px-2 py-0.5 text-xs rounded-r-lg transition-colors bg-[#1a1a1a] text-gray-400 hover:bg-green-600 hover:text-white`}
            onClick={handleDirectAutoBet}
            disabled={isPlacingBet || gameState === "active" || hasBet}
          >
            Auto
          </button>
        </div>
      ...
      {/* Betting Interface - Always show manual betting interface */}
            ...
            <div className="space-y-0.5">
              ...
              <div className="flex items-center gap-1">
                ...
                <div className="flex gap-1">
                  <button
                    className="w-6 h-6 bg-[#111111] rounded-full text-white flex items-center justify-center"
      ...
      {/* Auto Betting Settings - Hidden but still functional */}
