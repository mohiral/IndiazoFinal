...
const CrashGame = () => {
  ...
  const [resetBet, setResetBet] = useState(false)
  const [gameId, setGameId] = useState("game-" + Date.now())
  const [rocketVisible, setRocketVisible] = useState(true) // Added to ensure rocket visibility
  ...
  useEffect(() => {
    ...
    socket.on("game_state", (state) => {
      ...
      // If joining during active game, ensure rocket is visible and positioned correctly
      if (state.status === "active" && state.currentMultiplier) {
        setMultiplier(state.currentMultiplier)
        setGameActive(true)
        setCrashed(false)
        setRocketVisible(true)

        // Calculate path progress based on current multiplier
        const estimatedProgress = Math.min((state.currentMultiplier - 1) / 0.8, 1)
        ...
        // Make sure we reset properly for waiting state
        setGameActive(false)
        setCrashed(false)
        setPathProgress(0)
      }
    ...
    socket.on("game_started", () => {
      ...
      setSmokeParticles([])
      setRocketVisible(true) // Ensure rocket is visible when game starts
    })

    socket.on("multiplier_update", (newMultiplier) => {
      setMultiplier(newMultiplier)
      setRocketVisible(true) // Ensure rocket is visible on multiplier updates
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      console.error("Failed to load rocket image, using fallback")
      setRocketLoaded(true) // Still set to true so we render the fallback
    }
      ...
      if (gameState === "active") {
        ...
        setTimeout(() => {
          if (currentGameState === "active") {
            setGameActive(true)
            setRocketVisible(true) // Ensure rocket is visible after resize
          }
        }, 100)
      }
    }
  ...
  const handleCrash = () => {
    ...
    setGameActive(false)
    setRocketVisible(false) // Hide the main rocket during crash animation
  ...
  const calculatePartialPath = () => {
    ...
    // Always return at least the starting path
    const progress = Math.max(0.01, pathProgress)
    ...
    return `M 0 ${startY} Q ${controlX * progress} ${controlY + (startY - controlY) * (1 - progress)} ${x} ${y}`
  }
  ...
  const calculatePartialFillPath = () => {
    ...
    // Always return at least a minimal fill area
    const progress = Math.max(0.01, pathProgress)
    ...
    return `M 0 ${startY} Q ${controlX * progress} ${controlY + (startY - controlY) * (1 - progress)} ${x} ${y} L ${x} ${startY} L 0 ${startY}`
  }
      ...
      <div className="w-full h-auto min-h-[250px] md:min-h-[300px] bg-[#0a192f] p-2 sm:p-4 rounded-lg flex flex-col items-center relative border border-indigo-900 shadow-[0_0_15px_rgba(66,153,225,0.5)]">
        ...
        <div className="relative w-full h-48 sm:h-64 mt-2 overflow-visible">
          ...
          <svg className="absolute w-full h-full" style={{ zIndex: 1 }}>
            ...
            <path
              d={`M 0 ${window.innerWidth < 640 ? 160 : 240} L ${window.innerWidth < 640 ? 300 : 600} ${window.innerWidth < 640 ? 160 : 240}`}
              stroke="rgba(255,255,255,0.2)"
              strokeWidth="1"
              fill="none"
            />
            ...
            <path
              d={`M ${window.innerWidth < 640 ? 300 : 600} ${window.innerWidth < 640 ? 160 : 240} L ${window.innerWidth < 640 ? 300 : 600} 0`}
              stroke="rgba(255,255,255,0.2)"
              strokeWidth="1"
              fill="none"
            /> */}

            {/* Background that grows with the path - relaxed condition */}
            {(gameActive || gameState === "active") && !crashed && (
              <path
            ...
            {/* The curve path - relaxed condition */}
            {(gameActive || gameState === "active") && !crashed && (
              <path
            ...
            {/* Animated dotted lines - relaxed condition */}
            {(gameActive || gameState === "active") && !crashed && (
              <>
                <path
                  d={`M 0 0 L 0 ${window.innerWidth < 640 ? 160 : 240}`}
          ...
          {((gameState === "active" && rocketVisible) || (gameActive && !crashed)) && !explosionActive && (
            <div
              ...
              <div className="relative">
                ...
                {rocketLoaded ? (
                  <img
                    ...
                    onError={(e) => {
                      console.error("Error loading rocket image")
