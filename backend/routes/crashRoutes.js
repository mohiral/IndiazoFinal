...
// Add a new route to get a specific crash game by ID
router.get("/crash/game/:gameId", async (req, res) => {
  try {
    const { gameId } = req.params

    // Try to find the crash game with the exact ID
    let crashGame = await CrashHistory.findOne({ gameId }).lean()

    // If not found, try with "game-" prefix
    if (!crashGame && !gameId.startsWith("game-")) {
      crashGame = await CrashHistory.findOne({ gameId: `game-${gameId}` }).lean()
    }

    // If still not found, try without "game-" prefix
    if (!crashGame && gameId.startsWith("game-")) {
      crashGame = await CrashHistory.findOne({ gameId: gameId.substring(5) }).lean()
    }
